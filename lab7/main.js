...
function renderCesto(){
  ...
  Object.entries(cont).forEach(([id, qty])=>{
    ...
    menos.addEventListener('click', ()=>{
      const a = lerCarrinho();  const i = a.indexOf(Number(id));
      if (i > -1){ a.splice(i,1); gravarCarrinho(a); renderCesto(); }
    });
    mais.addEventListener('click', ()=>{
      const a = lerCarrinho();  a.push(Number(id));
      gravarCarrinho(a);  renderCesto();
    });
